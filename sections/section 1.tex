...
\begin{itemize}
    \item \textbf{Analysis}: determining the components by STFT or filterbank
...
A windowed signal transformation, on the other hand, suffers from \textbf{sidelobes} and \textbf{loss of resolution}:
...
The term $\Omega_M$ is the distance between the center of the main lobe and its first zero, for a rectangular window. So the main lobe width is defined in function of the rectangular one. 

\subsubsection{Window length \(M\) and frequency resolution \(\Delta\)}

In order to have a minimum \textbf{resolution requirement}, let's imagine we have two sinusoids with frequency difference $ \Delta $ (in Hz) in noisy conditions and we need a certain window length that guarantees to distinguish them in the frequency domain.
...
The bandwidth $B_W$ depends on the window parameter $L$ (factor of the $\Omega_M$), on the sampling frequency $F_s$ (in Hz) and on the \textbf{window length} $M$ (in samples):
...
A “type L” window of length $M$ or greater 
is said to resolve the sinusoidal 
frequencies $f_1$ and $f_2$.
Many signals are harmonic in nature, like tonal musical instruments and speech. It is shown in the spectrograms in Figure 1.9. 
...
For example, let us consider two singers (male and female) with respective fundamental frequency of $f_0 \ge 100$ Hz and $f_0 \ge 200$ Hz.
...
Working with sampled signals, a peak might fall slightly off its real location.
In this case, we want a \textbf{localization accuracy} requirement on the samples of the DFT: we may achieve that through \textbf{zero padding} (ideal interpolation, in time domain) or by \textbf{parabolic interpolation} (possibly in addition to zero padding).
...
Such a case is not acceptable: we need to zero pad the audio track, increasing its length from $M$ samples (window length) to a much longer length $N$ (DFT length).
...
The minimum frequency difference we want to detect must be less than the \textbf{Just Noticeable Difference (JND)}. 
The JND is the minimum frequency change in 
a pure tone (sinusoid) that we are able to perceive, approximated to:
...
\text{JND} =
\begin{cases}
     0.5\% \ f, & f \geq 600 \text{ Hz} \\
    3 \text{ Hz}, & f < 600 \text{ Hz}
\end{cases}
\]

In this example, the JND = 3 Hz, therefore the oversampling factor should be more than 8 (from 1024 to 8333), which means that the zero padding should make the time signal more than 8 times longer:
...
Zero padding in the time domain results in sinc or "bandlimited" interpolation in the frequency domain, but other interpolation forms may be used: one of those is the \textbf{parabolic interpolation}, which is particularly effective.
...
We assume a parabola centered in $p$, which is the parameter we want to compute. 
...
Remember that $p$ is the peak location (in samples) from the largest sample.
Using this, we have that the peak height estimate is $y(p)$.
...
Phase interpolation is independent.
...
Matlab has an unwrap function, and there are quite a few papers on the 
subject in the IEEE Transactions on (Acoustics, Speech, and) Signal 
Processing.

Alternatively, we may interpolate \textbf{real and imaginary part} separately and combine them to obtain the DFT (we remind that $X(f) \in \mathbb{C}$),  to 
yield a complex peak value estimate.
...
The \textbf{additive synthesis} technique with a bank of $ N $ oscillators results in an audio signal. It is a technique in which a signal is reconstructed from a summation of sinusoids and each sinusoid has a time varying amplitude and 
frequency :

\[ y(t) = \sum_{i=1}^N A_i(t) \sin \left[ \omega_i(t) + \phi_i(t) \right], \quad \omega_i(t) = 2 \pi f_i(t) \]

In order to reproduce a specific signal, we need to identify the sinusoidal components (\textbf{sound analysis}), specifically in terms of amplitude and frequency trajectories.
...
In addition to that, separating amplitude and frequency information allows to \textbf{easily implement time-scaling} and \textbf{frequency-shifting} operations (by simply resampling or changing the envelopes).

In the analysis with Short Time (fast) Fourier Transform (STFT), STFT can be interpreted as a filterbank: each bin of the
FFT is a band-pass filter for a specific frequency in time.
The FFT makes it computationally feasible to implement filter banks with a large number of analysis 
filters (changin the complexity from $O(N^2)$ to $O(N \log N)$).
...
We use quadratic interpolation (in the magnitude spectrum) to identify the spectrum magnitude peaks.
...
Linear interpolation may be used to define amplitude and frequency of each frequency component when phase is discarded (PARSHL, PARallel Sinusoidal Harmonic Line algorithm), while cubic interpolation may be used when the phase is retained (McAulay-Quatieri method).
...
In addition to that, noisy spectra are harder to manage for sure, and a transient detector may be used in order to indicate when the peak phases should be retained, for example using differentiated amplitude envelope of 
high-passed time-domain signal or linear prediction error.
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{Synthesis system.png}
    \caption{Phase-preserving synthesis system}
\end{figure}

In the \textbf{Spectral Modelling Synthesis} (SMS) we generalize the signal 
models used in PARSHL to include a filtered noise 
component. 
The time-varying spectrum of the signal is considered as made up by a \textbf{deterministic component} (sinusoids) and a \textbf{stochastic component} (time-varying noise):

\[ s(t) = \underbrace{\sum_{i=1}^N {A_i(t) \cos[\theta_i(t)]}}_\text{deterministic} + \underbrace{e(t)}_\text{stochastic} \]

where $A_i(t)$ and $\theta_i(t)$ are respectively the instantaneous amplitude and 
phase of the i-th sinusoidal component.
By assuming the random component $ e(t) $ as a \textbf{white noise} $ u(t) \sim \mathcal{N} (\mu, \sigma^2) $ \textbf{filtered} by $h_t$ ( $h_t(\tau)$ impulse response at time $\tau$ to an impulse at $t$ of a time varying linear filter):
...
\begin{figure}[H]
    \centering
    \includegraphics[width=0.65\linewidth]{Synthesis.png}
...
Signal domain is suitable for driving banks of oscillators because it is not undersampled over time.

The configuration of filterbanks analysis and bank of oscillators synthesis is the first ever obtained (in the late 1930s, in Bell Labs), even before computers were available, and it is the base of the \textbf{vocoder} (Voice Coder).
...
In the vocoder scheme, we divide the signal $x(t)$ into $N$ \textbf{frequency sub-intervals} (subbands): from each sub-signal $x_k(t)$ we compute magnitude (and optionally phase) spectrum for further elaborations.


Let us suppose that in each sub-interval (and sub-signal) we have only one sinusoid (that may have time-varying parameters).
...
$a_k(t)$ is an amplitude modulation term, $\omega_k$ is a fixed channel center frequency and $\phi_k(t)$ is a phase modulation term. Using these parameters, we can re-synthesize the signal using the oscillator summation.
...
The channel vocoder uses the amplitude only: a way to compute it is to apply an \textbf{envelope follower} to each sub-interval, using a rectifier (module of the signal, all the negative parts become positive) and a low-pass filter (averaging, because it removes the peaks and high parts, that were also the lowest parts, keeping the valuesin the middle).
The rectified signal has valleys and peaks: the low-pass filter sums that peaks in a certain time interval doing the average. The result is the smooth envelope line in red, that is the avergae energy in the subband.
...
This produces an approximation of the energy in each subband. 
This approach has been applied in parametric speech models. 
...
Under the assumption of no more than one sinusoid per subband, we can represent the signal in each channel as:
\[x_k(t) = a_k(t) \cos [\omega_k t + \phi_k(t)]\]
where $\omega_k$ is the center frequency of the k-th channel. 
We therefore need to extract two signals per channel: $a_k(t)$ that is  instantaneous amplitude (amplitude envelope) and $\phi_k(t)$ that is  instantaneous phase modulation.

Before discussing the phase vocoder, let us define some \textbf{equivalent representations} of narrow-band signals.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.4\linewidth]{Narrowband.png}
    \includegraphics[width=0.4\linewidth]{Analytic signal.png}
    \includegraphics[width=0.4\linewidth]{Baseband equivalent.png}
...
\[
\Re \{x_k^a(t) \} = a_k(t) \cos (\omega_k t + \phi_k(t)) \qquad 
\Im \{x_k^a(t) \} = a_k(t) \sin (\omega_k t + \phi_k(t))
\]
\[ a_k(t) = |x_k^a(t)| = \sqrt{\Re(x_k^a(t))^2 + \Im(x_k^a(t))^2}, \qquad \phi_k(t) = \angle{x_k^a(t)} - \omega_k t = \tan^{-1} \left[ \frac{\Im(x_k^a(t))}{\Re(x_k^a(t))} \right] - \omega_k t \]
...
\[ \mathrm{Im}(x_k^a(t)) \quad \overset{\mathcal{F}}{\longrightarrow} \quad \hat{X}(f) = \frac{1}{2j} \left[ X_a(f) - X_a^*(f) \right] = \frac{1}{2j} \left[ 1(f) - 1(-f) \right] X(f) = \underbrace{-j \ \sign(f)}_{G_H(f)} \ X(f) \]
...
\[
a_k(t) = \abs{x_k^m(t)} = \sqrt{\mathrm{Re}(x_k^m(t))^2 + \mathrm{Im}(x_k^m(t))^2}, \qquad \phi_k(t) = \angle{x_k^m(t)} = \tan^{-1}  =
...
Working with baseband signals, frequency deviation can be computed more easily, as the derivative of the instantaneous phase.
It is more convenient to extract the instantaneous frequency deviation, more than the phase, which is:
\[ \Delta\omega_k(t) = \frac{\partial \phi_k(t)}{\partial t} \quad \overset{\text{baseband equivalent}}{\longrightarrow} \quad \Delta\omega_k(t) = \frac{\partial \phi_k(t)}{\partial t} = \frac{\partial \angle{x_k^m(t)}}{\partial t} = \frac{x \dot{y} - y \dot{x}}{x^2 + y^2}\]
...
\begin{figure}[H]
    ...
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=\linewidth]{images/filterbanksstuff.png}
    \end{subfigure}
    \begin{subfigure}[t]{0.45\textwidth}
        \includegraphics[width=0.85\linewidth]{images/filterbanksstff2.png}
    \end{subfigure}
\end{figure}
